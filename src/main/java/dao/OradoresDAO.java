...
public class OradoresDAO {
    public void agregarOrador(Orador orador) {
        String sql = "INSERT INTO oradores (nombre, apellido, sugerencias , fecha_alta) VALUES (?, ?, ?, ?)";
        ...
        try (Connection conn = ConexionDB.conectar();
            ...
            pstmt.setString(3, orador.getSugerencias());
    ...
    public Orador obtenerPorId(int id) {
        ...
        try (Connection conn = ConexionDB.conectar();
            ...
            if (rs.next()) {
                ...
                orador.setSugerencias(rs.getString("sugerencias"));
    ...
    public void actualizarOrador(Orador orador) {
        String sql = "UPDATE oradores SET nombre = ?, apellido = ?, sugerencias = ?, fecha_alta = ? WHERE id_orador = ?";
        try (Connection conn = ConexionDB.conectar();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
